...
results_file = f"/data/PRTNR/CHUV/RADMED/phagmann/hcp/scaling_features/simple_sessions-{n}.h5"
...
for sub in subjects:
    ...
    variability = np.array([MSSD(ts[i]) for i in range(ts.shape[0])])
    fALFF = compute_fALFF(ts)
    hurst_e = np.array([hurst.compute_Hc(ts[i])[0] for i in range(ts.shape[0])])
    measures = [mean, std, variability, fALFF, hurst_e]
    measure_names = ["mean", "std", "variability", "fALFF", "hurst"]
